...
C3_flag = 1;     % 1: C=DSC_NET; 2: C learned by GMC LRSSC 
                  
delta = 4;       % post-processing of angular data adjacency matrix ( delta > 1: 4 or 8)      

dimSubspace = 7; % subspace dimension for ORL dataset
...
for it=1:numit
    ...
    for l=1:nc
        ind = randperm(10);
        ind_in = (l-1)*10 + ind(1:7);
        labels_in = [labels_in, ones(1,7)*l];
    ...
    for i=1:length(alpha)
        ...
        for j=1:length(lambda)
            for k=1:length(gamma_gmc)
                ...
                CE0(it,i,j,k)  = computeCE(labels_est,labels);
                
                [C, error] = ADMM_LRSSC(normc(X1_in),options);
                A = adjacency_matrix_angular_domain(C, delta);
                labels_est = SpectralClustering_shifted_Laplacian(A,nc);
                CE1(it,i,j,k)  = computeCE(labels_est,labels);
                
                [C, error] = ADMM_LRSSC(normc(X2_in),options);
                A = adjacency_matrix_angular_domain(C, delta);
                labels_est = SpectralClustering_shifted_Laplacian(A,nc);
                CE2(it,i,j,k)  = computeCE(labels_est,labels);
                
                if C3_flag == 2
                    [C, error] = ADMM_LRSSC(normc(X3_in),options);
                    A = adjacency_matrix_angular_domain(C, delta);
                    labels_est = SpectralClustering_shifted_Laplacian(A,nc);
                    CE3(it,i,j,k)  = computeCE(labels_est,labels);
                end
            end
        end
    end
...
for iter=1:numit
    ...
    for l=1:nc
        ...
        % out-of sample data
        ind_out = (l-1)*10 + ind(8:10);
        labels_out = [labels_out, ones(1,3)*l];
