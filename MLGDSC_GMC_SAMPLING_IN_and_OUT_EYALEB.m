%% MLGDSC_GMC_SAMPLING_IN_and_OUT_EYALEB (Multilayer graph deep subspace clustering)
...
dimSubspace = 9;    % a priori known (presumed) dimension of the subspace
...
%% data information
i1=48; i2=42; N=2432; nc=38; % 2432 images of 38 faces (64 images per face) (each image is 48x42 pixels)

% OVAJ DIO ZAVISI OD TOGA KAKO SU FORMATIRANI PODACI 
% UAZNA MATRICA X0 je dimenzija je 2016x2432
% input data (layer 0) 
load YaleBCrop025.mat;
X = zeros(i1*i2, N); % allocation of space
...
clear Y
X0 = X0'
% OVAJ DIO ZAVISI KAKO SU FORMATIRANI PODACI - Matrice su dimenzija x20432 broj uzoraka 
% layer 1 output
X1=csvread('yale1.csv');
X1 = transpose(X1); % AKO TREBAAAAAAAAAAA

% layer 2 output
X2=csvread('yale2.csv');
X2 = transpose(X2); % AKO TREBAAAAAAAAAAA

% layer 3 output
X3=csvread('yale3.csv');
X3 = transpose(X3); % AKO TREBAAAAAAAAAAA

% self-representation matrix learned by DSC-L2 net algorithm
C_DSC_L2_net = csvread('yale-matc.csv'); 
...
for run = 1:nruns
    ...
    for l=1:nc
        ind = randperm(64);
        ind_in = (l-1)*64 + ind(1:45);  % 45 images for in-sample dataset
        labels_in = [labels_in, ones(1,45)*l];
...
save MLG_GMC_EYALEB_hyperparameters lambda_0 alpha_0 gamma_0 lambda_1 alpha_1 gamma_1...
...
for it=1:numit
    ...
    for l=1:nc
        ...
        ind_out = (l-1)*64 + ind(46:64);
        index_out = [index_out, ind_out];
        labels_in = [labels_in, ones(1,45)*l];
        labels_out = [labels_out, ones(1,19)*l];
...
save MLG_EYALEB_Results ACC_MLG_in NMI_MLG_in Fscore_MLG_in Rand_MLG_in Purity_MLG_in ...
